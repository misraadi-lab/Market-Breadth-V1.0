...
const INDEXES = [
  ...
  { key: "N500", title: "NIFTY 500", symbol: "^CRSLDX" }, // fallback symbol proxy if CNX500 unavailable
  { key: "MID", title: "NIFTY MIDCAP", symbol: "^CRSMID" }, // Yahoo tickers vary; we expose a dropdown to change
  { key: "SMALL", title: "NIFTY SMALLCAP", symbol: "^CNSMALLCAP" },
  { key: "MICRO", title: "NIFTY MICROCAP", symbol: "^NIFMICRO" }, // may not exist; allow user override
];

const TF = [
  { key: "daily", label: "1D", interval: "1d", range: "6mo" },
  { key: "h1", label: "60m", interval: "60m", range: "10d" },
  { key: "m15", label: "15m", interval: "15m", range: "5d" },
];
...
const Pill = ({ value }) => {
  ...
  return <span className={`px-2 py-0.5 text-xs rounded-full ${bg} border border-white/10`}>{value.toFixed(1)}</span>;
};
...
function BlockCard({ title, symbol, maLen, tfOn }) {
  ...
  const score = useMemo(()=>{
    const get = (c)=>{
      ...
      const s = rollingSMA(c, maLen);
      return perTFScore(c, s);
    };
    const sD = tfOn.daily ? 2*get(d1.candles) : 0;
    const sH = tfOn.h1 ? 1.5*get(h1.candles) : 0;
    const sM = tfOn.m15 ? 1*get(m15.candles) : 0;
    return sD + sH + sM;
  }, [d1.candles, h1.candles, m15.candles, maLen, tfOn]);
  ...
  return (
    <div className="bg-slate-900/60 border border-white/10 rounded-2xl p-4 flex flex-col gap-3">
      ...
      <div className="grid grid-cols-3 gap-3">
        {tfOn.daily && d1.candles && <TFCard tfLabel="1D" candles={d1.candles} maLen={maLen} />}
        {tfOn.h1 && h1.candles && <TFCard tfLabel="60m" candles={h1.candles} maLen={maLen} />}
        {tfOn.m15 && m15.candles && <TFCard tfLabel="15m" candles={m15.candles} maLen={maLen} />}
      </div>
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white">
        ...
        <aside className="bg-slate-900/60 border border-white/10 rounded-2xl p-4 h-fit sticky top-20">
          ...
          <div className="space-y-4">
            ...
            <div className="pt-2 border-t border-white/10 text-xs text-white/60">
              Uses free Yahoo Finance data (delayed). You can edit symbols to exact tickers later if you prefer alternative proxies.
            </div>
          </div>
        </aside>
      </main>
      ...
      <footer className="max-w-7xl mx-auto px-4 pb-8 text-xs text-white/40">
        © {new Date().getFullYear()} Market Breadth — Yahoo
      </footer>
    </div>
  );
}
